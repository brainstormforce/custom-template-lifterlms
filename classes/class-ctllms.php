	...
	class CTLLMS {
		...
		public function get_action_content( $post_id ) {
			...
			if ( class_exists( 'FLBuilderModel' ) ) {
				...
				if ( $do_render && $fl_enabled ) {
					...
					if ( is_callable( 'FLBuilderShortcodes::insert_layout' ) ) {
						echo FLBuilderShortcodes::insert_layout( // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
							array(
								'id' => $post_id,
							)
						);
			...
			if ( self::is_tve_activated( $post_id ) ) {
				ob_start();
				echo apply_filters( 'the_content', $post->post_content ); //phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
				wp_reset_postdata();
				return ob_get_clean();
			}
