	...
	class CTLLMS_Admin {
		...
		public function active_admin_menu() {
			...
			if ( ( 'post-new.php' === $pagenow || 'post.php' === $pagenow ) && 'bsf-custom-template' === $current_screen->post_type ) :
				$submenu_file = 'edit.php?post_type=bsf-custom-template'; //phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
				$parent_file  = 'lifterlms';//phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
			endif;
		...
		public function custom_post_type_post_update_messages( $messages ) {
			...
			if ( 'bsf-custom-template' === $custom_post_type ) {
				...
				$messages[ $custom_post_type ] = array(
					...
					5  => isset( $_GET['revision'] ) ? sprintf( __( '%1$s restored to revision from %2$s', 'custom-template-lifterlms' ), $singular_name, wp_post_revision_title( (int) $_GET['revision'], false ) ) : false, //phpcs:ignore WordPress.Security.NonceVerification.Recommended
					/* translators: %s: singular custom post type name */
		...
		public function save_course_landing_page( $post_id ) {
			...
			$landing_page_id = ( isset( $_POST['course_template'] ) ) ? absint( $_POST['course_template'] ) : ''; //phpcs:ignore WordPress.Security.NonceVerification.Missing
